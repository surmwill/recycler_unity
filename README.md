...
To prevent unnecessary recalculations, we disable all `ILayoutGroups` and `ILayoutElements` on the entry except during the small windows of binding and manual size recalculations (see [RecalculateDimensions](https://github.com/surmwill/recycler_unity/tree/master#recalculatedimensions)). This ensures no size recalculation of the child's subtree on adding, removing, and modifying entries - and performance. However, disabled layout elements mixed with a `VerticalLayoutGroup` also controlling that layout will result in a default 0 width and height on the next layout pass. The entry evaporates, and the `VerticalLayoutGroup` must not `childControlWidth` or `childControlHeight`.
